...
def task_info():
    ...
    st.markdown("Для вирішення даної задачі були використані дані з відкритих джерел, серед яких \
                **макроекономічні показники України** та **щоденні зніми курсу гривні до USD**")
    features_info = pd.DataFrame(['індекс цін виробників', 'індекс інфляції', 'золотовалютні резерви', 'індекс цін сільськогосподарської продукції',
                                               'ВВП на душу населення грн', 'рівень безробіття', 'платіжний баланс (дол)', 'виконання державного бюджету(сальдо)',
    ...
    st.subheader("Використані бібліотеки мови Python:")
...
def print_about_economic_features_model():
    ...
    st.markdown("Вся робота проходила із середнім місячним курсом, адже усі залежні показники вимірюються кожного місяця/кварталу/року.")
    ...
    st.markdown("Валідація проводилась на основі **TimeSeriesSplit** із бібліотеки sklearn, щоб перевірити як модель себе показує на всьому датасеті. На діаграмі проілюстрований принцип розбиття на тренувальну та тестову частини.")
...
def select_fs():
    ...
    st.markdown("**А які підводні камені?**\n- не завжди може дати бажаний результат через наявність екстремальних значень\n- хибний вибір функції, яка на тренувальних даних зробила вагомий внесок у формування прогнозу")
    sеlect_selection = st.radio("Оберіть необіхне", ["Без відбору", "З відбором"])
