...
def select_predict():
    ...
    if value:
            y_test, y_train, preds, mae, X_test, X_train, model, tm = work_model(predict_size, select, sеlect_step, fs)
            print_res(y_test, preds, mae, X_test, predict_size, tm)
...
def select_interp():
    ...
    if value:
        y_test, y_train, preds, mae, X_test, X_train, model, tm = work_model(predict_size, "Звичайне навчання", 0, fs)
